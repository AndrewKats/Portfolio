  ...
  (type-case ExprC a
    ...
    [restC (a) (type-case Value (interp a env)
                 [listV (elems) (if (empty? elems)
                                     ...
                                     (listV (rest elems)))]
                 [else (error 'interp "not a list")])]
    [if0C (con thn els) (type-case Value (interp con env)
                          [numV (n)(cond
                                     [(= n 0) (interp thn env)]
                                     [else (interp els env)])]
...
(module+ test
  ...
  (test/exn (interp (parse '{let {[bad : (num -> num) {lambda {[x : num]} {+ x y}}]}
                    ...
                    mt-env)
            "free variable")
  (test/exn (interp (parse '(if0 empty 1 2)) mt-env)
            "not a number"))
          ...
          (local [(define result-type (varT (box (none))))]
            (begin
              (unify! (arrowT (typecheck arg tenv)
                      ...
                      fun)
              result-type))]
    [if0C (con thn els) (local [(define result-type (varT (box (none))))]
                          (begin
                            (unify! (typecheck con tenv) (numT) con)
                            (unify! (typecheck thn tenv) result-type thn)
                            (unify! (typecheck els tenv) result-type els)
                            result-type))]
    ;; Fixed
    [emptyC () (listofT (varT (box (none))))]
    [consC (l r) (local [(define result-type (varT (box (none))))]
                          (begin
                            (unify! (typecheck l tenv) result-type l)
                            (unify! (typecheck r tenv) (listofT result-type) r)
                            (listofT result-type)))]
    [firstC (a) (local [(define result-type (varT (box (none))))]
                          (begin
                            (unify! (typecheck a tenv) (listofT result-type) a)
                            result-type))]
...
(module+ test
  ...
  (test (resolve (typecheck (parse '(if0 0 1 2)) mt-env))
        (numT))
...
(define (run-prog [exp : s-expression]) : s-expression
  (let ([parsed-exp (parse exp)])
    (let ([resolved-type (resolve (typecheck parsed-exp mt-env))])
    (begin (let ([interped (interp parsed-exp mt-env)])
             (cond
               [(numT? resolved-type) (number->s-exp (numV-n interped))]
               [(arrowT? resolved-type) `function]
               [(listofT? resolved-type) `list]))))))
...
(module+ test
  ...
  ;; run-prog function
  (test (run-prog '{lambda {[x : num]} {+ x x}})
        `function)
