...
(module+ test
  ...
  (test (parse '(instanceof (new thatclass 1) thatclass))
        (instanceofI (newI 'thatclass (list (numI 1))) 'thatclass))
  (test (parse '(cast otherclass (new thatclass 1)))
        (castI 'otherclass (newI 'thatclass (list (numI 1)))))
  (test (parse `null)
        (nullI))
  (test/exn (parse `x)
            "invalid input")
