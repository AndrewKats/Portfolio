    ...
    (type-case ExprI a
      ...
      [superI (method-name arg-expr)
              (ssendC (thisC)
                      ...
                      (recur arg-expr))]
      [if0I (con thn els)
            (if0C (recur con) (recur thn) (recur els))]
      [instanceofI (obj name)
                   (instanceofC (recur obj) name)]
      [castI (cname exp)
             (castC cname (recur exp))]
      [nullI () (nullC)])))
  ...
  (type-case ClassC c
    [classC (name super-name field-names methods)
            (type-case ClassC (flatten-super name classes i-classes)
              [classC (super-name super-super-name super-field-names super-methods)
                      (classC
                       name
                       super-name
