...
(module+ test
  ...
  (test (parse '(box 5)) (boxC (numC 5)))
  (test (parse '(unbox (box 5))) (unboxC (boxC (numC 5))))
...
;; Add a case for boxV? (and new conts if they exist)
(define (update)
  (if (= updated-ptr ptr)
...
(define (interp)
  (case (code-ref expr-reg 0)
    ...
    [(91) ;box
     (begin
       ;(set! v-reg (malloc1 81 (code-ref expr-reg 1)))
       (display "box\n")
       (set! k-reg (malloc1 61 k-reg))
       (set! expr-reg (code-ref expr-reg 1))
       (interp))]
    [(92) ;unbox
     (begin
       (display "unbox\n")
       (set! k-reg (malloc1 62 k-reg))
       (set! expr-reg (code-ref expr-reg 1))
       (interp))
     
       #;(case (code-ref v-reg 0)
         [(15) (begin
                 (set! v-reg (malloc1 15 (code-ref v-reg 1)))
                 (interp))]
         [(16) (begin 
                 (set! v-reg (malloc1 16 (code-ref v-reg 1)))
                 (interp))]
         [(81) (begin
                 (set! v-reg (malloc1 81 (code-ref v-reg 1)))
                 (interp))]
         [else
          (begin
            (display "\n")
            (display (code-ref v-reg 0))
            (display "\n")
            (error 'interp "sheeeit"))])])) 
  ...
  (case (ref k-reg 0)
    ...
    [(61) ;doBoxK
     (begin
       (display "doBoxK\n")
       #;(case (code-ref v-reg 0)
         [(15) (set! v-reg (malloc1 15 (code-ref v-reg 1)))]
         [(16) (set! v-reg (malloc2 16 (code-ref v-reg 1) (code-ref v-reg 2)))]
         [(81) (set! v-reg (malloc1 81 (code-ref v-reg 1)))])
       (set! v-reg (malloc1 81 v-reg))
       #;(set! env-reg (ref k-reg 2))
    ...
    [(62) ;doUnboxK
     (begin
       (display "doUnboxK\n")
       #;(case (code-ref v-reg 0)
         [(15) (set! v-reg (code-ref))]
         [(16)]
         [(81)])
       (set! v-reg (code-ref (code-ref v-reg 1) 1))
       (set! k-reg (ref k-reg 1))
       (continue))]))
