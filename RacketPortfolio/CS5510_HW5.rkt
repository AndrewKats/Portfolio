    ...
    [(s-exp-match? '{letrec {[SYMBOL ANY]} ANY} s)
     (let ([bs {s-exp->list (first (s-exp->list (second (s-exp->list s))))}])
       (appC (lamC (s-exp->symbol (first bs))
             ...
             (appC (parse mk-rec-fun) (lamC (s-exp->symbol (first bs)) (parse (second bs))))))]
...
(module+ test
  ...
  (test (interp (parse '{letrec ([sumup {lambda (n)
                                        (if0 n
                                             0
                                             (+ n (sumup (+ -1 n))))}])
                          (sumup 5)})
  ...
  ;; times tests
  (test (interp (parse (list->s-exp (list (list->s-exp (list times '2)) '3))) mt-env)
        (numV (* 2 3)))
  (test (interp (parse (list->s-exp (list (list->s-exp (list times '2)) '3))) mt-env)
        (numV 6))
  (test (interp (parse (list->s-exp (list (list->s-exp (list times '0)) '3))) mt-env)
        (numV (* 0 3)))
  (test (interp (parse (list->s-exp (list (list->s-exp (list times '0)) '3))) mt-env)
        (numV 0))
  ...
  (test/exn (interp (parse '{let {[bad {lambda {x} {+ x y}}]}
                    ...
                    mt-env)
            "free variable")
  (test/exn (num-zero? (closV 'n (numC 1) mt-env))
            "not a number")
