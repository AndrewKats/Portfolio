...
(define (interp [a : ExprC] [env : Env]) : Value
  (type-case ExprC a
    ...
    [firstC (arg) (type-case Value (interp arg env)
                    (consV (l r env-c) (interp l env-c))
                    (else (error 'interp "not a cons")))]
    [restC (arg) (type-case Value (interp arg env)
                    (consV (l r env-c) (interp r env-c))
                    (else (error 'interp "not a cons")))]
    [consC (l r) (consV l r env)]))
...
(module+ test
  ...
  (test (num-zero? (numV 0))
        #t)
  (test (num-zero? (interp (parse '(+ 1 -1)) mt-env))
        #t)
  (test (num-zero? (numV 1))
        #f)
  (test (num-zero? (interp (parse '(+ 1 2)) mt-env))
        #f)
  (test/exn (num-zero? (consV (numC 1) (numC 2) mt-env))
   "not a number"))
...
(module+ test
  ...
  (test/exn (interp-expr (parse '(first 1)))
        "not a cons")
  (test/exn (interp-expr (parse '(rest 1)))
        "not a cons")
  )
