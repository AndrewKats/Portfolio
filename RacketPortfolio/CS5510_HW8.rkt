...
(define-type ExprC
  ...
  [ssendC (obj-expr : ExprC)
          ...
          (method-name : symbol)
          (arg-expr : ExprC)]
  [selectC (result : ExprC)
           (obj-expr : ExprC)]
  [instanceofC (class : ExprC)
               (potential-super-name : symbol)])
    ...
    (local [(define (recur expr)
      ...
      (type-case ExprC a
        ...
        [newC (class-name field-exprs)
              (local [(define c (find-class class-name (append (list (classC 'object empty empty 'object)) classes)))
        ...
        [ssendC (obj-expr class-name method-name arg-expr)
                (local [(define obj (recur obj-expr))
                  ...
                  (call-method class-name method-name classes
                               obj arg-val))]
        [selectC (res obj) (type-case Value (recur res)
                             [numV (n) (cond
                                         [(= n 0) (type-case Value (recur obj)
                                                    [objV (name fields) (call-method name 'zero classes (recur obj) (numV 0))]
                                                    [else (error 'interp "not an object")])]
                                         [else (type-case Value (recur obj)
                                                    [objV (name fields) (call-method name 'nonzero classes (recur obj) (numV 0))]
                                                    [else (error 'interp "not an object")])])]
                             [else (error 'interp "not a number")])]
        [instanceofC (class-exp maybe-super) (cond
                                           [(is-instance-of (objV-class-name (recur class-exp)) maybe-super classes) (numV 0)]
                                           [else (numV 1)])]))))

(define (is-instance-of [class-name : symbol] [super-name : symbol] [classes : (listof ClassC)]) : boolean
  (cond
    [(eq? class-name super-name) #t]
    [(eq? (classC-super-name (find-class class-name classes)) super-name) #t]
    [(eq? (classC-super-name (find-class class-name classes)) 'object) #f]
    [else (is-instance-of (classC-super-name (find-class class-name classes)) super-name classes)]))
    ...
    (type-case ExprI a
      ...
      [superI (method-name arg-expr)
              (ssendC (thisC)
                      ...
                      method-name
                      (recur arg-expr))]
      [selectI (res class-exp) (selectC (recur res) (recur class-exp))]
      [instanceofI (class-exp maybe-super) (instanceofC (recur class-exp) maybe-super)])))
...
(module+ test
  ...
  (test (expr-i->c (selectI (numI 0) (newI 'object empty)) 'object)
        (selectC (numC 0) (newC 'object empty)))
  (test (expr-i->c (instanceofI (newI 'object empty) 'object) 'object)
        (instanceofC (newC 'object empty) 'object))
  )
  ...
  (type-case ClassC c
    [classC (name field-names methods super-name)
            (type-case ClassC (flatten-super name classes i-classes)
              [classC (super-name super-field-names super-methods super-super-name)
                      (classC
                       name
...
(module+ test
  ...
  (test (parse '{super m 1})
        (superI 'm (numI 1)))
  (test (parse '(new object))
   (newI 'object empty))
  (test (parse '{select 0 {new snowball 1}})
        (selectI (numI 0) (newI 'snowball (list (numI 1)))))
  (test (parse '{instanceof {new fish 1 2} fish})
        (instanceofI (newI 'fish (list (numI 1) (numI 2))) 'fish))
  (test/exn (parse `x)
            "invalid input")
...
;; =============================================================
  ...
  ;; MORE NEW TESTS
  (test (interp (selectC (numC 0) (newC 'snowball (list (numC 1))))
                (list (classC
                       'snowball
                       (list 'size)
                       (list
                        (methodC 'zero (thisC))
                        (methodC 'nonzero (newC 'snowball (list (plusC (numC 1) (getC (thisC) 'size))))))
                       'object))
                (numV -1)
                (numV -1))
  ...
  (test (interp-prog (list '{class fish extends object
                           ...
                           '{class shark extends object
                                   {teeth}})
                     '{instanceof {new shark 1} fish}) 
        '1)

  (test (interp-prog (list '{class animal extends object
                                   {wings}}
                           '{class fish extends animal
                                   {size color}}
                           '{class shark extends fish
                                   {teeth}})
                     '{instanceof {new shark 0 1 2 3} animal}) 
        '0)

  (test/exn (interp (selectC (numC 0) (numC 0)) empty (numV -1) (numV -1))
            "not an object")
  
  (test/exn (interp (selectC (numC 1) (numC 0)) empty (numV -1) (numV -1))
            "not an object")

  (test/exn (interp (selectC (newC 'object empty) (numC 0)) empty (numV -1) (numV -1))
            "not a number")

  (test (is-instance-of 'object 'object empty)
        #t)
  ...
  (test (interp-prog (list
                      ...
                      '(class b extends a ())
                      '(class c extends b () (make-a (new a))))
         '(instanceof (send (new c) make-a 0) b))
   '1)
